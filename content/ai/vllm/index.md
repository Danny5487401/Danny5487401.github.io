...
2. CNN（Convolutional Neural Network 卷积神经网络): 适用于处理图像、视频等二维或三维数据。通过卷积层捕捉局部特征，池化层进行下采样，减少参数数量。
应用：图像识别、目标检测、图像生成等。

3. RNN（ Recurrent Neural Networks 循环神经网络 )：具有循环结构，能够处理序列数据和时序依赖关系。神经元的输出可以作为自身的输入，记忆先前状态的信息。
应用：文本生成、语音识别、机器翻译等
