...
summary: "一个智能体编排框架，包裹在 Claude 模型外面。它提供工具、上下文管理和执行环境，把一个语言模型变成一个有能力的编码 Agent。"
categories:
  - ai
---
...
|   模型   | 行为 | 备注 |
|:------:|:--:|::| 
| sonnet |  使用最新的 Sonnet 模型（当前为 Sonnet 4.5）用于日常编码任务  | 无 |
|  opus  | 使用最新的 Opus 模型（当前为 Opus 4.6）用于复杂推理任务 | 无 |
| haiku  | 使用快速高效的 Haiku 模型用于简单任务 | 无 |
